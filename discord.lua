...
function DiscordLib:Window(data)
    ...
    local function InitAutoLoad()
        ...
        elseif data.RememberLastConfig ~= false then
        ...
        function ChannelHold:Channel(text)
            ...
            function ChannelContent:Toggle(flag, config)
                ...
                Toggle.MouseButton1Click:Connect(function()
                    UpdateToggle(not toggled, false)
                end)
            
                -- ✅ PERBAIKAN: SetValue function yang lebih robust
                function ToggleFunc:SetValue(val)
                    if type(val) == "boolean" then
                        UpdateToggle(val, true)  -- silent = true untuk avoid callback loop
                    else
                        warn("[Toggle] SetValue: Invalid value type, expected boolean, got " .. type(val))
                    end
                end
                
                -- ✅ PERBAIKAN: Register SEBELUM set initial value
                if flag then
                ...
                if config.Value then
                    UpdateToggle(config.Value, true)
                end

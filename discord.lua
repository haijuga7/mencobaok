...
function DiscordLib.ConfigSystem:LoadConfig(name)
    local success, data = pcall(function()
        return game:GetService("HttpService"):JSONDecode(readfile(self.ConfigFolder.."/"..name..".json"))
    end)
    if success and data then
        for flag, value in pairs(data) do
            if DiscordLib.Flags[flag] and DiscordLib.Flags[flag].SetValue then
...
-- ============================================
-- IMPROVED BOTTOM DRAG BAR + CORNER RESIZE HANDLE
-- ============================================
local function AddBottomControls(mainFrame, topFrame, data)
    -- ============================================
    -- BOTTOM DRAG BAR (TRANSPARAN, DI BAWAH MAINFRAME)
    -- ============================================
    local BottomDragBar = Instance.new("Frame")
    BottomDragBar.Name = "BottomDragBar"
    BottomDragBar.Parent = mainFrame.Parent -- Parent ke Discord ScreenGui, bukan mainFrame
    BottomDragBar.AnchorPoint = Vector2.new(0.5, 0)
    BottomDragBar.BackgroundTransparency = 1 -- TRANSPARAN
    BottomDragBar.BorderSizePixel = 0
    BottomDragBar.Position = UDim2.new(mainFrame.Position.X.Scale, mainFrame.Position.X.Offset, 
                                        mainFrame.Position.Y.Scale, mainFrame.Position.Y.Offset + mainFrame.Size.Y.Offset)
    BottomDragBar.Size = UDim2.new(0, mainFrame.Size.X.Offset, 0, 35)
    BottomDragBar.ZIndex = 5

    -- Update posisi BottomDragBar saat mainFrame berubah posisi
    mainFrame:GetPropertyChangedSignal("Position"):Connect(function()
        BottomDragBar.Position = UDim2.new(mainFrame.Position.X.Scale, mainFrame.Position.X.Offset, 
                                            mainFrame.Position.Y.Scale, mainFrame.Position.Y.Offset + mainFrame.Size.Y.Offset)
    end)
    
    -- Update ukuran BottomDragBar saat mainFrame di-resize
    ...
    -- Icon garis-garis untuk drag (seperti BloxFishHub)
    local DragIcon = Instance.new("Frame")
    DragIcon.Name = "DragIcon"
    DragIcon.Parent = BottomDragBar
    DragIcon.AnchorPoint = Vector2.new(0.5, 0.5)
    DragIcon.BackgroundTransparency = 1
    DragIcon.Position = UDim2.new(0.5, 0, 0.5, 0)
    DragIcon.Size = UDim2.new(0, 50, 0, 10)

    -- Buat 3 baris horizontal (drag indicator seperti BloxFishHub)
    for i = 1, 3 do
        local Line = Instance.new("Frame")
        Line.Name = "Line" .. i
        Line.Parent = DragIcon
        Line.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
        Line.BorderSizePixel = 0
        Line.Position = UDim2.new(0, 0, (i - 1) * 0.4, 0)
    ...
    -- ⚡ ENABLE BOTTOM BAR DRAG (masih drag mainFrame)
    MakeDraggable(BottomDragBar, mainFrame)

    -- ============================================
    -- VERSION LABEL (di drag bar)
    -- ============================================
    if data.Version then
        local VersionLabel = Instance.new("TextLabel")
        VersionLabel.Name = "VersionLabel"
        VersionLabel.Parent = BottomDragBar
        VersionLabel.BackgroundTransparency = 1
        VersionLabel.Position = UDim2.new(0.02, 0, 0, 0)
    ...
    -- ============================================
    -- RESIZE HANDLE (Pojok kanan bawah seperti BloxFishHub)
    -- ============================================
    local ResizeHandle
    if data.Resizable then
        ResizeHandle = Instance.new("TextButton")
        ResizeHandle.Name = "ResizeHandle"
        ResizeHandle.Parent = mainFrame.Parent -- Parent ke Discord ScreenGui
        ResizeHandle.AnchorPoint = Vector2.new(1, 1)
        ResizeHandle.BackgroundColor3 = Color3.fromRGB(114, 137, 228)
        ResizeHandle.BackgroundTransparency = 0.3
        ResizeHandle.BorderSizePixel = 0
        ResizeHandle.Position = UDim2.new(mainFrame.Position.X.Scale, mainFrame.Position.X.Offset + mainFrame.Size.X.Offset + 8, 
                                          mainFrame.Position.Y.Scale, mainFrame.Position.Y.Offset + mainFrame.Size.Y.Offset + 43)
        ResizeHandle.Size = UDim2.new(0, 25, 0, 25)
        ResizeHandle.ZIndex = 10
        ResizeHandle.AutoButtonColor = false
        ResizeHandle.Text = ""

        -- Update posisi ResizeHandle saat mainFrame berubah
        mainFrame:GetPropertyChangedSignal("Position"):Connect(function()
            ResizeHandle.Position = UDim2.new(mainFrame.Position.X.Scale, mainFrame.Position.X.Offset + mainFrame.Size.X.Offset + 8, 
                                              mainFrame.Position.Y.Scale, mainFrame.Position.Y.Offset + mainFrame.Size.Y.Offset + 43)
        end)
        ...
        -- Icon resize (3 garis diagonal seperti BloxFishHub)
        local ResizeIconFrame = Instance.new("Frame")
        ResizeIconFrame.Name = "ResizeIconFrame"
        ResizeIconFrame.Parent = ResizeHandle
        ResizeIconFrame.BackgroundTransparency = 1
        ResizeIconFrame.Size = UDim2.new(1, 0, 1, 0)
        ResizeIconFrame.ZIndex = 11
        
        -- Buat 3 garis diagonal
        for i = 1, 3 do
            local DiagonalLine = Instance.new("Frame")
            DiagonalLine.Name = "DiagonalLine" .. i
            DiagonalLine.Parent = ResizeIconFrame
            DiagonalLine.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            DiagonalLine.BackgroundTransparency = 0.4
            DiagonalLine.BorderSizePixel = 0
            DiagonalLine.Position = UDim2.new(0, 5 + (i - 1) * 5, 0, 15 - (i - 1) * 5)
            DiagonalLine.Size = UDim2.new(0, 12, 0, 2)
            DiagonalLine.Rotation = 45
            DiagonalLine.ZIndex = 11
            
            local LineCorner = Instance.new("UICorner")
            LineCorner.CornerRadius = UDim.new(1, 0)
            LineCorner.Parent = DiagonalLine
        end
        ...
        local startSize = nil
        local minSize = data.MinSize or UDim2.new(0, 400, 0, 300)
        local maxSize = data.MaxSize or UDim2.new(0, 1200, 0, 800)
        ...
        ResizeHandle.MouseEnter:Connect(function()
            ...
            for _, child in pairs(ResizeIconFrame:GetChildren()) do
    ...
    -- ============================================
    -- HOVER EFFECTS untuk Bottom Bar
    -- ============================================
    BottomDragBar.MouseEnter:Connect(function()
        for _, child in pairs(DragIcon:GetChildren()) do
            if child:IsA("Frame") then
                TweenService:Create(
                    child,
                    TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
    ...
    BottomDragBar.MouseLeave:Connect(function()
        for _, child in pairs(DragIcon:GetChildren()) do
            if child:IsA("Frame") then
                TweenService:Create(
                    child,
    ...
    return BottomDragBar, ResizeHandle
end
...
function DiscordLib:Window(data)
    ...
    local AutoLoadSystem = {
        ...
        ConfigName = data.ConfigName or "AutoSave",
    ...
    local function InitAutoLoad()
        ...
        local configToLoad = nil
    
        -- Prioritas 1: AutoLoad yang diset di data window
        if AutoLoadSystem.Enabled and AutoLoadSystem.ConfigName then
            configToLoad = AutoLoadSystem.ConfigName
        -- Prioritas 2: Config terakhir yang digunakan
        elseif data.RememberLastConfig ~= false then -- default true
            configToLoad = DiscordLib.ConfigSystem:GetLastConfig()
        end
        ...
        -- Setup auto-save jika diaktifkan
        if AutoLoadSystem.AutoSave then
            spawn(function()
                while task.wait(AutoLoadSystem.AutoSaveInterval) do
                    local nameToSave = AutoLoadSystem.ConfigName or DiscordLib.ConfigSystem:GetLastConfig() or "AutoSave"
                    DiscordLib.ConfigSystem:SaveConfig(nameToSave)
                    print("[DiscordLib] Auto-saved config:", nameToSave, "at", os.date("%X"))
                end
            end)
            
            -- Save on player leave
            game:GetService("Players").LocalPlayer.AnchorPointChanged:Connect(function()
                local nameToSave = AutoLoadSystem.ConfigName or DiscordLib.ConfigSystem:GetLastConfig() or "AutoSave"
                DiscordLib.ConfigSystem:SaveConfig(nameToSave)
            end)
        end
    ...
    local bottomBar, resizeHandle = AddBottomControls(MainFrame, TopFrame, data)

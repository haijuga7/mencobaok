-- ============================================
-- ULTRA-OPTIMIZED CONFIG SYSTEM (Zero Freeze)
-- ============================================
local DiscordLib = {}
DiscordLib.Flags = {}
DiscordLib.ConfigSystem = {
    ...
    -- ✅ Cache untuk avoid multiple reads
    _configCache = {},
    _loadInProgress = false,
    _saveQueue = {}
}

-- ✅ COMPRESSION: Hanya simpan nilai yang berubah dari default
...
local function CompressConfig(data)
    ...
    print("[Config] Compressed:", count, "flags")
    return compressed
end
...
-- ✅ INSTANT SAVE: Queue-based system (non-blocking)
function DiscordLib.ConfigSystem:SaveConfig(name)
    ...
    if not self._saveInProgress then
        ...
        spawn(function()
            task.wait(0.1) -- Small delay to batch multiple saves
...
-- ✅ INSTANT LOAD: Direct flag setting (no batching needed)
function DiscordLib.ConfigSystem:LoadConfig(name)
    if self._loadInProgress then
        warn("[Config] Load already in progress, skipping...")
        return false
    end
    ...
    spawn(function()
        local success, data = pcall(function()
            ...
            if self._configCache[name] then
                print("[Config] Using cached data for:", name)
        ...
        if success and data then
            print("[Config] Loading", name, "- Flags:", #data)
            
            -- ✅ INSTANT APPLY: Set all flags at once (no delay)
            local appliedCount = 0
            
            for flag, value in pairs(data) do
                -- ✅ Direct pcall per flag (fast & safe)
                ...
                if not ok then
                    -- Silent fail for missing flags (script might have changed)
                end
            end
            
            print("[Config] ✅ Applied", appliedCount, "flags instantly")
            self._loadInProgress = false
            return true
        else
            warn("[Config] ❌ Failed to load:", name)
            self._loadInProgress = false
            return false
        end
    end)
...
-- ✅ CLEAR CACHE (Call ini jika memory penuh)
...
function DiscordLib:Window(data)
    ...
    local function InitAutoLoad()
        ...
        -- ✅ INSTANT START: Tidak perlu delay lama
        task.wait(0.5) -- Cukup 0.5 detik untuk UI ready
        
        local configToLoad = DiscordLib.ConfigSystem:GetLastConfig()
        
        if configToLoad then
            print("[DiscordLib] 🚀 Fast-loading config:", configToLoad)
            
            -- ✅ INSTANT LOAD (Non-blocking)
            DiscordLib.ConfigSystem:LoadConfig(configToLoad)
            
            -- ✅ Show notification after 0.5s (jangan tunggu load selesai)
            ...
            print("[DiscordLib] No saved config found")
            if AutoLoadSystem.ShowNotification then
                task.delay(0.5, function()
                    pcall(function()
                        DiscordLib:Notification(
        ...
        -- ✅ AUTO-SAVE: Background thread dengan smart interval
        if AutoLoadSystem.AutoSave then
            spawn(function()
                ...
                while true do
                    ...
                    -- ✅ Queue save (instant, non-blocking)

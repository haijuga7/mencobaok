...
-- ✅ PROGRESSIVE LOAD: Load dalam chunks kecil (prevent freeze)
function DiscordLib.ConfigSystem:LoadConfig(name, silentMode)
    ...
    spawn(function()
        ...
        -- ✅ PROGRESSIVE LOADING: Bagi jadi chunks kecil
        local flagList = {}
        for flag, value in pairs(data) do
            table.insert(flagList, {flag = flag, value = value})
        end
        
        local totalFlags = #flagList
        print("[Config] 📦 Loading", totalFlags, "flags progressively...")
        
        -- ✅ Load dalam batch ultra-kecil (5 flag per frame)
        local batchSize = 5
        ...
        local function loadNextBatch()
            if currentIndex > totalFlags then
                -- ✅ SELESAI
            ...
            -- ✅ Load batch current
            local endIndex = math.min(currentIndex + batchSize - 1, totalFlags)
            ...
            -- ✅ YIELD ke frame berikutnya (ini yang bikin smooth!)
            task.wait()

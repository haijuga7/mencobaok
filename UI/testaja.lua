-- ============================================
-- HYPER-OPTIMIZED CONFIG SYSTEM (Per-Account Auto-Load Only)
-- ============================================
local DiscordLib = {}
...
DiscordLib.ConfigSystem = {
    ...
    LastConfigFile = "LastConfig.json",
...
-- ✅ HELPER: Check if value should be saved
local function ShouldSaveValue(value)
...
-- ✅ COMPRESSION: Only save non-default values
local function CompressConfig(data)
    local compressed = {}
    local count = 0
    
    for flag, value in pairs(data) do
        if ShouldSaveValue(value) then
            compressed[flag] = value
            count = count + 1
        end
    end
    
    print("[Config] Compressed to", count, "flags")
    return compressed
end

-- ✅ HELPER: Get player-specific folder path (ONLY for auto-load)
local function GetPlayerAutoLoadFolder()
...
function DiscordLib.ConfigSystem:GetFlag(flag)
    return self.Flags[flag]
end

-- ✅ ASYNC SAVE: Queue-based non-blocking save (GLOBAL CONFIG)
function DiscordLib.ConfigSystem:SaveConfig(name)
    table.insert(self._saveQueue, {
        name = name,
        timestamp = tick()
    })
    ...
    if not self._saveInProgress then
        ...
        spawn(function()
            ...
            while #self._saveQueue > 0 do
                local saveData = table.remove(self._saveQueue, 1)
                ...
                local success = pcall(function()
                    ...
                    -- ✅ SAVE TO GLOBAL FOLDER (not per-player)
                    local filePath = self.ConfigFolder .. "/" .. saveData.name .. ".json"
                    writefile(filePath, encoded)
                    
                    print("[Config] ✅ Saved:", saveData.name)
                end)
                
                if not success then
                    warn("[Config] ❌ Save failed:", saveData.name)
                end
...
-- ✅ LOAD CONFIG (GLOBAL)
function DiscordLib.ConfigSystem:LoadConfig(name, silentMode)
    if self._loadInProgress then
        warn("[Config] Load already in progress")
        return false
    end
    ...
    spawn(function()
        ...
        if not success then
            warn("[Config] ❌ Load failed:", name)
            self._loadInProgress = false
            return
        end
        
        if not data then
            warn("[Config] ❌ No data found")
            self._loadInProgress = false
            return
        end
        
        -- ✅ PHASE 1: Load SEMUA DROPDOWN dulu
        print("[Config] 📦 Phase 1: Loading all dropdowns...")
        local dropdownCount = 0
        local otherFlags = {}
        
        for flag, value in pairs(data) do
            local element = DiscordLib.Flags[flag]
            
            -- Cek apakah ini dropdown (punya function Refresh atau Add)
            if element and (element.Refresh or element.Add or element.Clear) then
                local ok = pcall(function()
                    if element.SetValue then
                        element:SetValue(value)
                        dropdownCount = dropdownCount + 1
                        print("[Config] ✅ Dropdown loaded:", flag)
                    end
                end)
                
                if not ok then
                    warn("[Config] ❌ Failed dropdown:", flag)
                end
                
                task.wait(0.03) -- Small delay antar dropdown
            else
                -- Simpan untuk phase 2
                table.insert(otherFlags, {flag = flag, value = value})
            end
        end
        
        print("[Config] ✅ Phase 1 complete! Loaded", dropdownCount, "dropdowns")
        task.wait(0.2) -- Wait untuk dropdown callbacks selesai
        
        -- ✅ PHASE 2: Load element lainnya (batch processing)
        print("[Config] 📦 Phase 2: Loading remaining", #otherFlags, "items...")
        
        local batchSize = 5
        local currentIndex = 1
        local successCount = dropdownCount
        local failCount = 0
        
        local function loadNextBatch()
            if currentIndex > #otherFlags then
                print("[Config] ✅ Load complete!", successCount, "success,", failCount, "failed")
                self._loadInProgress = false
            ...
            local endIndex = math.min(currentIndex + batchSize - 1, #otherFlags)
            
            for i = currentIndex, endIndex do
                local item = otherFlags[i]
                
                local ok = pcall(function()
                    if DiscordLib.Flags[item.flag] and DiscordLib.Flags[item.flag].SetValue then
...
-- ✅ AUTO LOAD CONFIG (PER-PLAYER FOLDER)
function DiscordLib.ConfigSystem:AutoLoadConfig(name)
    local success = pcall(function()
        local playerFolder = GetPlayerAutoLoadFolder()
        local lastConfigPath = playerFolder .. "/" .. self.LastConfigFile
        
        writefile(lastConfigPath, name)
        print("[Config] ✅ Auto-load set for", plr.Name .. ":", name)
    end)
    
    return success
end

-- ✅ GET LAST CONFIG (PER-PLAYER FOLDER)
function DiscordLib.ConfigSystem:GetLastConfig()
    ...
    return success and configName or nil
end

-- ✅ DELETE CONFIG (GLOBAL)
function DiscordLib.ConfigSystem:DeleteConfig(name)
    self._configCache[name] = nil
...
-- ✅ LIST CONFIGS (GLOBAL)
function DiscordLib.ConfigSystem:ListConfigs()
    local configs = {}
    pcall(function()
        if not isfolder(self.ConfigFolder) then
            makefolder(self.ConfigFolder)
        ...
        if isfolder(self.ConfigFolder) then
            for _, file in ipairs(listfiles(self.ConfigFolder)) do
                -- ✅ SKIP player folders (only get .json files in root)
                if file:match("%.json$") then
                    local configName = file:gsub(self.ConfigFolder .. "/", ""):gsub("%.json$", "")
                    
                    -- ✅ SKIP if it's inside a player folder (has underscore pattern)
                    if not configName:match("_") then
...
-- ✅ CLEAR CACHE
function DiscordLib.ConfigSystem:ClearCache()
    self._configCache = {}
    collectgarbage("collect")
    print("[Config] 🧹 Cache cleared + GC collected")
end
...
function DiscordLib:Window(data)
    ...
    local AutoLoadSystem = {
        Enabled = data.AutoLoad or false,
        AutoSave = data.AutoSave or false,
        AutoSaveInterval = data.AutoSaveInterval or 60,
        ShowNotification = data.ShowNotification ~= false -- default true
    }
    
    local function InitAutoLoad()
        if not AutoLoadSystem.Enabled then return end
        
        -- ✅ Delay minimal untuk UI ready
        task.wait(1)
        
        local configToLoad = DiscordLib.ConfigSystem:GetLastConfig()
        
        if configToLoad then
            print("[DiscordLib] 🚀 Auto-loading:", configToLoad)
            
            -- ✅ Silent mode (no notification during auto-load)
            DiscordLib.ConfigSystem:LoadConfig(configToLoad, true)
            
            -- ✅ Show notification setelah 2 detik
            if AutoLoadSystem.ShowNotification then
        ...
        if AutoLoadSystem.AutoSave then
            spawn(function()
                local saveInterval = AutoLoadSystem.AutoSaveInterval or 60
    ...
    CloseBtn.Position = UDim2.new(0.959063113, 0, -0.0169996787, 0) -- Tetap di kanan
    ...
    MinimizeBtn.Position = UDim2.new(0.917947114, 0, -0.0169996787, 0) -- Tetap posisi
    ...
    local FloatingIconGlow = Instance.new("ImageLabel")
    local FloatingIconImage = Instance.new("ImageLabel")
    
    FloatingIcon.Name = "FloatingIcon"
    FloatingIcon.Parent = Discord
    ...
    FloatingIconGlow.Name = "Glow"
    FloatingIconGlow.Parent = FloatingIcon
    FloatingIconGlow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    FloatingIconGlow.BackgroundTransparency = 1.000
    FloatingIconGlow.Position = UDim2.new(0, -15, 0, -15)
    FloatingIconGlow.Size = UDim2.new(1, 30, 1, 30)
    FloatingIconGlow.ZIndex = 998
    FloatingIconGlow.Image = "rbxassetid://4996891970"
    FloatingIconGlow.ImageColor3 = Color3.fromRGB(15, 15, 15)
    FloatingIconGlow.ScaleType = Enum.ScaleType.Slice
    FloatingIconGlow.SliceCenter = Rect.new(20, 20, 280, 280)
    
    FloatingIconImage.Name = "IconImage"
    FloatingIconImage.Parent = FloatingIcon
    FloatingIconImage.AnchorPoint = Vector2.new(0.5, 0.5)
    FloatingIconImage.BackgroundTransparency = 1.000
    FloatingIconImage.Position = UDim2.new(0.5, 0, 0.5, 0)
    FloatingIconImage.Size = UDim2.new(0, 35, 0, 35)
    FloatingIconImage.Image = "rbxassetid://6034509993" -- Discord logo atau icon pilihan
    ...
    -- ============================================
    -- MINIMIZE LOGIC WITH FLOATING ICON
    -- ============================================
    MinimizeBtn.MouseButton1Click:Connect(function()
        if minimized == false then
            ...
            -- Show floating icon with animation
            FloatingIcon.Visible = true
            FloatingIcon.Size = UDim2.new(0, 0, 0, 0)
            FloatingIcon:TweenSize(
                UDim2.new(0, 60, 0, 60),
            ...
            spawn(function()
                while FloatingIcon.Visible do
                    for i = 0, 360, 2 do
                        if not FloatingIcon.Visible then break end
                        FloatingIconImage.Rotation = i
                        task.wait(0.01)
                    end
                end
            end)
    ...
    -- ============================================
    -- FLOATING ICON CLICK TO RESTORE
    -- ============================================
    ...
    FloatingIcon.MouseEnter:Connect(function()
        TweenService:Create(
            ...
            {Size = UDim2.new(0, 70, 0, 70), BackgroundColor3 = Color3.fromRGB(103, 123, 196)}
        ):Play()
    end)
    ...
    FloatingIcon.MouseLeave:Connect(function()
        TweenService:Create(
            FloatingIcon,
            TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {Size = UDim2.new(0, 60, 0, 60), BackgroundColor3 = Color3.fromRGB(114, 137, 228)}
        ):Play()
    end)
    ...
    function ServerHold:Server(text, img)
        ...
        function ChannelHold:Channel(text)
            ...
            function ChannelContent:Button(config)
            ...
            function ChannelContent:Toggle(flag, config)
                local ToggleFunc = {}
                local toggled = config.Value or false
                
                -- ✅ REGISTER FLAG SEGERA di awal function
                if flag then
                    -- Buat object sementara dengan SetValue function
                    DiscordLib.Flags[flag] = {
                        SetValue = function(self, val)
                            -- Placeholder, akan di-override nanti
                            toggled = val
                        end
                    }
                end
                ...
                -- ✅ Function untuk update visual toggle
                ...
                Toggle.MouseButton1Click:Connect(function()
                    ...
                    if flag then
                        DiscordLib.ConfigSystem:SetFlag(flag, toggled)
                    end
                    
                    pcall(config.Callback, toggled)
                end)
            
                -- ✅ SetValue function yang BENAR
                function ToggleFunc:SetValue(val)
                    toggled = val
                    UpdateVisual(val)
                    
                    if flag then
                        DiscordLib.ConfigSystem:SetFlag(flag, val)
                    end
                ...
                if flag then
                    DiscordLib.Flags[flag] = ToggleFunc
                    DiscordLib.ConfigSystem:SetFlag(flag, toggled)
                end
            ...
            function ChannelContent:Slider(flag, config)
                local SliderFunc = {}
                if flag then
                    DiscordLib.Flags[flag] = {
                        SetValue = function(self, val)
                ...
                CurrentValueFrame.Size = UDim2.new((config.Start or 0) / config.Max, 0, 0, 8)
                ...
                Zip.Position = UDim2.new((config.Start or 0)/config.Max, -6,-0.644999981, 0)
                ...
                ValueLabel.Text = tostring(config.Start and math.floor((config.Start / config.Max) * (config.Max - config.Min) + config.Min) or 0)
                ...
                local function move(input)
                    ...
                    Zip.Position = pos
                    local value = math.floor(((pos.X.Scale * config.Max) / config.Max) * (config.Max - config.Min) + config.Min)  -- ✅ FIX
                ...
                function SliderFunc:SetValue(tochange)
                    ...
                    if flag then
                        DiscordLib.ConfigSystem:SetFlag(flag, tochange)
                    end
                    pcall(config.Callback, tochange)
                end
                
                ChannelHolder.CanvasSize = UDim2.new(0,0,0,ChannelHolderLayout.AbsoluteContentSize.Y)
                
                if flag then
                    DiscordLib.Flags[flag] = SliderFunc
                    DiscordLib.ConfigSystem:SetFlag(flag, config.Start or 0)
                end
    ...
    -- ============================================
            -- DROPDOWN WITH SIDE PANEL (CHLOE STYLE) - FIXED SETVALUE
            -- Fix untuk auto load config dan GetValue
            -- ============================================
            
            function ChannelContent:Dropdown(flag, config)
                local DropdownFunc = {}
                local selectedValues = {}
                local allOptions = config.List or {}
                local isOpen = false
                local isInitialized = false -- Track initialization
                
                -- Main Dropdown Button (tidak expand ke bawah)
                ...
                -- Side Panel (overlay di kanan channel, full height)
                local SidePanel = Instance.new("Frame")
                local SidePanelCorner = Instance.new("UICorner")
                local SidePanelBorder = Instance.new("UIStroke")
                local SidePanelShadow = Instance.new("Frame")
                ...
                -- Setup Main Dropdown (TIDAK EXPAND)
                Dropdown.Name = "Dropdown"
                Dropdown.Parent = ChannelHolder
                Dropdown.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
                Dropdown.BorderSizePixel = 0
                Dropdown.Size = UDim2.new(0, 401, 0, 60) -- Fixed height, tidak expand
                ...
                DropdownTitle.TextColor3 = Color3.fromRGB(220, 221, 222)
                ...
                DropdownButton.BackgroundColor3 = Color3.fromRGB(32, 34, 37)
                ...
                DropdownButtonText.BackgroundTransparency = 1.000
                DropdownButtonText.Position = UDim2.new(0, 8, 0, 0)
                DropdownButtonText.Size = UDim2.new(1, -35, 1, 0)
                DropdownButtonText.Font = Enum.Font.Gotham
                DropdownButtonText.Text = config.Default or "Select..."
                DropdownButtonText.TextColor3 = Color3.fromRGB(185, 187, 190)
                DropdownButtonText.TextSize = 13.000
                DropdownButtonText.TextXAlignment = Enum.TextXAlignment.Left
                DropdownButtonText.TextTruncate = Enum.TextTruncate.AtEnd
                DropdownButtonText.ZIndex = 101
                
                -- Arrow Icon
                DropdownArrow.Name = "DropdownArrow"
                DropdownArrow.Parent = DropdownButton
                DropdownArrow.BackgroundTransparency = 1.000
                DropdownArrow.Position = UDim2.new(1, -25, 0, 5)
                DropdownArrow.Size = UDim2.new(0, 15, 0, 15)
                DropdownArrow.Image = "rbxassetid://6031090997"
                DropdownArrow.ImageColor3 = Color3.fromRGB(185, 187, 190)
                DropdownArrow.ZIndex = 101
                
                -- ============================================
                -- SIDE PANEL (OVERLAY DI KANAN)
                -- ============================================
                
                -- Shadow/Overlay Background (darkens main area)
                SidePanelShadow.Name = "SidePanelShadow"
                SidePanelShadow.Parent = ChannelHolder.Parent -- Parent ke ChannelFrame
                SidePanelShadow.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
                SidePanelShadow.BackgroundTransparency = 1.0
                SidePanelShadow.BorderSizePixel = 0
                SidePanelShadow.Position = UDim2.new(0, 0, 0, 0)
                SidePanelShadow.Size = UDim2.new(1, 0, 1, 0)
                SidePanelShadow.Visible = false
                SidePanelShadow.ZIndex = 1000
                
                -- Side Panel (di kanan)
                SidePanel.Name = "SidePanel_" .. (config.Title or "Dropdown")
                SidePanel.Parent = ChannelHolder.Parent -- Parent ke ChannelFrame
                SidePanel.BackgroundColor3 = Color3.fromRGB(32, 34, 37)
                SidePanel.BorderSizePixel = 0
                -- Start dari kanan (hidden)
                SidePanel.Position = UDim2.new(1, 0, 0, 0) -- Start offscreen right
                SidePanel.Size = UDim2.new(0, 0, 1, 0) -- Width 0 initially, full height
                SidePanel.ClipsDescendants = true
                SidePanel.Visible = false
                SidePanel.ZIndex = 1001
                
                SidePanelCorner.CornerRadius = UDim.new(0, 6)
                SidePanelCorner.Parent = SidePanel
                
                SidePanelBorder.Name = "Border"
                SidePanelBorder.Parent = SidePanel
                SidePanelBorder.Color = Color3.fromRGB(45, 47, 52)
                SidePanelBorder.Thickness = 2
                SidePanelBorder.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                
                -- Panel Header
                PanelHeader.Name = "PanelHeader"
                PanelHeader.Parent = SidePanel
                PanelHeader.BackgroundColor3 = Color3.fromRGB(40, 42, 46)
                PanelHeader.BorderSizePixel = 0
                PanelHeader.Position = UDim2.new(0, 0, 0, 0)
                PanelHeader.Size = UDim2.new(1, 0, 0, 45)
                PanelHeader.ZIndex = 1002
                ...
                PanelTitle.Text = config.Title or "Select Option"
                PanelTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
                PanelTitle.TextSize = 15.000
                PanelTitle.TextXAlignment = Enum.TextXAlignment.Left
                PanelTitle.ZIndex = 1002
                
                -- Close Button (X)
                CloseButton.Name = "CloseButton"
                CloseButton.Parent = PanelHeader
                CloseButton.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
                ...
                CloseButton.MouseEnter:Connect(function()
                    CloseButton.BackgroundColor3 = Color3.fromRGB(237, 66, 69) -- Red on hover
                end)
                ...
                SearchContainer.Parent = SidePanel
                SearchContainer.BackgroundColor3 = Color3.fromRGB(40, 42, 46)
                ...
                SearchBox.PlaceholderColor3 = Color3.fromRGB(114, 118, 125)
                SearchBox.Text = ""
                ...
                -- Options Scroll (FULL HEIGHT - search ke bawah)
                OptionsScroll.Name = "OptionsScroll"
                OptionsScroll.Parent = SidePanel
                OptionsScroll.BackgroundTransparency = 1.000
                OptionsScroll.BorderSizePixel = 0
                OptionsScroll.Position = UDim2.new(0, 10, 0, 100)
                OptionsScroll.Size = UDim2.new(1, -20, 1, -110) -- Full height minus header & search
                OptionsScroll.ScrollBarThickness = 4
                OptionsScroll.ScrollBarImageColor3 = Color3.fromRGB(79, 84, 92)
                OptionsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
                OptionsScroll.ZIndex = 1002
                ...
                local function CreateOption(value)
                    ...
                    OptionButton.BackgroundColor3 = Color3.fromRGB(40, 42, 46)
                    ...
                    OptionText.TextColor3 = Color3.fromRGB(220, 221, 222)
                    OptionText.TextSize = 13.000
                    OptionText.TextXAlignment = Enum.TextXAlignment.Left
                    OptionText.TextTruncate = Enum.TextTruncate.AtEnd
                    OptionText.ZIndex = 1003
                    
                    OptionCheckmark.Name = "Checkmark"
                    OptionCheckmark.Parent = OptionButton
                    OptionCheckmark.BackgroundTransparency = 1.000
                    OptionCheckmark.Position = UDim2.new(0, 10, 0, 0)
                    OptionCheckmark.Size = UDim2.new(0, 20, 1, 0)
                    OptionCheckmark.Font = Enum.Font.GothamBold
                    OptionCheckmark.Text = ""
                    OptionCheckmark.TextColor3 = Color3.fromRGB(88, 101, 242)
                    OptionCheckmark.TextSize = 15.000
                    OptionCheckmark.ZIndex = 1003
                    
                    -- Hover effect
                    OptionButton.MouseEnter:Connect(function()
                        OptionButton.BackgroundTransparency = 0.000
                    ...
                    OptionButton.MouseButton1Click:Connect(function()
                        if config.Multi then
                            ...
                            local selected = {}
                            for v, _ in pairs(selectedValues) do
                                table.insert(selected, v)
                            end
                            
                            DropdownButtonText.Text = #selected > 0 and table.concat(selected, ", ") or (config.Default or "Select...")
                            
                            if flag then
                                DiscordLib.Flags[flag] = selected
                                DiscordLib.ConfigSystem:SetFlag(flag, selected)
                            end
                ...
                -- Refresh options
                local function RefreshOptions(searchQuery)
                    for _, child in pairs(OptionsScroll:GetChildren()) do
                    ...
                    for _, value in ipairs(allOptions) do
                        if searchQuery == "" or value:lower():find(searchQuery, 1, true) then
                            table.insert(filteredOptions, value)
                        end
                    end
                    
                    for _, value in ipairs(filteredOptions) do
                        local optionButton = CreateOption(value)
                        if selectedValues[value] then
                            local checkmark = optionButton:FindFirstChild("Checkmark")
                            if checkmark then checkmark.Text = "✓" end
                        end
                    end
                    ...
                    if #filteredOptions == 0 then
                        ...
                        NoResults.Parent = OptionsScroll
                        NoResults.BackgroundTransparency = 1.000
                        NoResults.Size = UDim2.new(1, 0, 0, 50)
                        NoResults.Font = Enum.Font.GothamSemibold
                        NoResults.Text = "No results found"
                        NoResults.TextColor3 = Color3.fromRGB(114, 118, 125)
                        NoResults.TextSize = 12.000
                        NoResults.ZIndex = 1003
                    end
                end
                ...
                function DropdownFunc:Open()
                    isOpen = true
                    SidePanelShadow.Visible = true
                    SidePanel.Visible = true
                    
                    -- Panel width: 280px (cukup lebar tapi tidak menutup semua)
                    ...
                    game:GetService("TweenService"):Create(
                        ...
                        {
                            Position = UDim2.new(1, -panelWidth - 10, 0, 10),
                ...
                SidePanelShadow.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        DropdownFunc:Close()
                    end
                end)
                ...
                -- ============================================
                -- API FUNCTIONS (FIXED untuk Auto Load)
                -- ============================================
                
                function DropdownFunc:SetValue(value)
                    -- Validasi: pastikan value ada dalam list
                    local function isValidValue(val)
                        return table.find(allOptions, val) ~= nil
                    end
                        ...
                        if type(value) == "table" then
                            for _, v in ipairs(value) do
                                if isValidValue(v) then
                                    ...
                                    warn("[Dropdown] Invalid value in multi-select:", v)
                                end
                            end
                            
                            local validSelected = {}
                            for v, _ in pairs(selectedValues) do
                                table.insert(validSelected, v)
                            end
                            
                            DropdownButtonText.Text = #validSelected > 0 and table.concat(validSelected, ", ") or (config.Default or "Select...")
                        else
                            DropdownButtonText.Text = config.Default or "Select..."
                        end
                    else
                        -- Single select
                        if type(value) == "string" and isValidValue(value) then
                            selectedValues = {[value] = true}
                            DropdownButtonText.Text = value
                        elseif type(value) == "table" and #value > 0 and isValidValue(value[1]) then
                            -- Handle jika config load return table untuk single select
                            selectedValues = {[value[1]] = true}
                            DropdownButtonText.Text = value[1]
                        else
                            selectedValues = {}
                            DropdownButtonText.Text = config.Default or "Select..."
                            if value ~= nil and value ~= "" then
                                warn("[Dropdown] Invalid value for single-select:", value)
                            end
                        end
                    end
                    
                    -- Update visual jika sudah initialized
                    if isInitialized then
                        RefreshOptions("")
                    end
                    
                    -- Save to flag (PENTING: simpan format yang benar)
                    if flag then
                        local saveValue
                        if config.Multi then
                            saveValue = {}
                            for v, _ in pairs(selectedValues) do
                                table.insert(saveValue, v)
                            end
                        else
                            for v, _ in pairs(selectedValues) do
                                saveValue = v
                        ...
                        DiscordLib.Flags[flag] = saveValue
                        DiscordLib.ConfigSystem:SetFlag(flag, saveValue)
                    end
                    
                    -- NO callback saat SetValue (untuk avoid loop saat load config)
                    return true
                end
                ...
                function DropdownFunc:GetValue()
                    if config.Multi then
                        ...
                        -- Single select - return string langsung
                        for v, _ in pairs(selectedValues) do
                            return v
                        end
                        return nil
                ...
                function DropdownFunc:Clear()
                    ...
                    if flag then
                        DiscordLib.Flags[flag] = config.Multi and {} or nil
                        DiscordLib.ConfigSystem:SetFlag(flag, config.Multi and {} or nil)
                    end
                end
                
                function DropdownFunc:Refresh(newList)
                    allOptions = newList or {}
                    
                    -- Re-validate selected values
                    local newSelectedValues = {}
                    for v, _ in pairs(selectedValues) do
                        if table.find(allOptions, v) then
                            newSelectedValues[v] = true
                        end
                    end
                ...
                function DropdownFunc:Add(value)
                    if not table.find(allOptions, value) then
                ...
                function DropdownFunc:Remove(value)
                    local index = table.find(allOptions, value)
                    if index then
                        table.remove(allOptions, index)
                        if selectedValues[value] then
                            selectedValues[value] = nil
                ...
                if config.Default then
                    DropdownFunc:SetValue(config.Default)
                end
                ...
                -- Register to flags (PENTING: register dropdown object, bukan value)
                if flag then
                    DiscordLib.Flags[flag] = DropdownFunc
                end
            ...
            function ChannelContent:Colorpicker(flag, config)
                if flag then
                    DiscordLib.Flags[flag] = {
                ...
                ColorSelection.Position = UDim2.new(config.Preset and select(3, Color3.toHSV(config.Preset)) or UDim2.new(0.5, 0, 0.5, 0))
                ...
                DiscordLib.ConfigSystem:SetFlag(flag, {Color3.toHSV(config.Preset or Color3.new(1,1,1))})
                HueSelection.Size = UDim2.new(0, 18, 0, 18)
                HueSelection.Image = "http://www.roblox.com/asset/?id=4805639000"

                PresetClr.Name = "PresetClr"
                PresetClr.Parent = ColorpickerFrame
                PresetClr.BackgroundColor3 = config.Preset
                ...
                local function UpdateColorPicker(nope)
                    ...
                    pcall(config.Callback, PresetClr.BackgroundColor3)
                end
                ...
                PresetClr.BackgroundColor3 = config.Preset or Color3.new(1,1,1)
                Color.BackgroundColor3 = config.Preset or Color3.new(1,1,1)
                pcall(config.Callback, PresetClr.BackgroundColor3)
                ...
                function ColorpickerFunc:SetValue(color)
                    local h, s, v = Color3.toHSV(color)
                    ColorH, ColorS, ColorV = h, s, v
                    -- Update UI colorpicker disini
                    PresetClr.BackgroundColor3 = color
                    Color.BackgroundColor3 = Color3.fromHSV(h, 1, 1)
                    
                    if flag then
                        DiscordLib.ConfigSystem:SetFlag(flag, {h, s, v})
                    end
                    pcall(config.Callback, color)
                end
                
                if flag then
                    DiscordLib.Flags[flag] = ColorpickerFunc
                    DiscordLib.ConfigSystem:SetFlag(flag, {Color3.toHSV(config.Preset or Color3.new(1,1,1))})
                end
            ...
            function ChannelContent:Textbox(flag, config)
                local TextboxFunc = {}
    
                -- ✅ REGISTER FLAG SEGERA
                if flag then
                    DiscordLib.Flags[flag] = {
                        SetValue = function(self, val)
                            -- Placeholder
                ...
                function TextboxFunc:SetValue(val)
                    TextBox.Text = val
                    if flag then
                        DiscordLib.ConfigSystem:SetFlag(flag, val)
                    end
                ...
                TextBox.FocusLost:Connect(function(ep)
                    if ep and #TextBox.Text > 0 then
                        if flag then
                            DiscordLib.ConfigSystem:SetFlag(flag, TextBox.Text)
                        end
                ...
                if flag then
                    DiscordLib.Flags[flag] = TextboxFunc
                end
            ...
            function ChannelContent:Bind(flag, config)
                ...
                KeybindText.Text = config.Value and config.Value.Name or "None"

local DiscordLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/haijuga7/mencobaok/refs/heads/main/UI/testUIv2.lua"))()
            ...
            if b then
                task.spawn(function()
                    while autoERodState do
                        pcall(function() RE_EquipToolFromHotbar:FireServer(1) end) task.wait(0.5)
                    end
                end)
            end
        end
...
do 
    local peler = GetRemote("RE/EquipToolFromHotbar")
    ...
    local stuckTog = fishMain:Toggle("1stuckTog", {
        Title = "Anti Stuck",
        Value = false,
        Callback = function(val)
            stuckActive = val
    ...
    local BlatantV1Toggle = fishMain:Toggle("1togblatv1", { -- ✅ DIPERBAIKI
...
do 
    ...
    local function checkFilter(isName, isMutasi, isRarity, iip)
        if #customName > 0 and #customMutation then
            if table.find(customName, isName) and table.find(customMutation, isMutasi) then
                return iip
            end 
        end
            
        if #customMutation > 0 and #customRarity > 0 then 
            if table.find(customMutation, isMutasi) and table.find(customRarity, isRarity) then
                return iip
            end
        end
        
        if isMutasi == 'Gemstone' and isName == "Ruby" then
            return iip
        end
        
        if #selectedRarities > 0 and table.find(selectedRarities, isRarity) then
            return true
        end
    
        -- 2. Cek Nama (Hanya jika filter nama dipilih)
        -- Kita pakai 'if not isMatch' biar gak double check kalau udah match di rarity
        if #selectedItemNames > 0 and table.find(selectedItemNames, isName) then
            return true
        end
    
        -- 3. Cek Mutasi (Hanya jika filter mutasi dipilih)
        if #selectedMutations > 0 and table.find(selectedMutations, isMutasi) then
            return true
        end
    ...
    local function favoriteONFIshCaught(itemid, metadata, fulldata)
        if not autoFavoriteState then return end
        ...
        local uuid = fulldata['InventoryItem']['UUID']
        
        local testbb = checkFilter(fishName, mutationString, fishRarity, true)
    ...
    local function GetItemsToFavorite()
        ...
        for _, item in ipairs(inventoryData.Items) do
            ...
            local isMatch = checkFilter(name, mutationFilterString, rarity, true)
    ...
    local function GetItemsToUnfavorite()
        ...
        for _, item in ipairs(inventoryData.Items) do
            ...
            local isTargetedForUnfavorite = checkFilter(name, mutationFilterString, rarity, false)
    ...
    if REObtainedNewFishNotification then
        REObtainedNewFishNotification.OnClientEvent:Connect(function(itemId, metadata, fullData)
            pcall(function() favoriteONFIshCaught(itemId, metadata, fullData) end)
        end)
    end
    ...
    local MutationDropdown = fishFav:Dropdown("dmut", {
        Title = "by Mutation",
        List = {"Shiny", "Festive", "Gemstone", "Corrupt", "Galaxy", "Holographic", "Ghost", "Lightning", "Fairy Dust", "Gold", "Midnight", "Radioactive", "Stone", "Albino", "Sandy", "Acidic", "Disco", "Frozen", "Noob"},
        Multi = true, 
        Callback = function(val)
    ...
    local togglefav = fishFav:Toggle("tvav", {
        Title = "Enable Auto Favorite on Fish Caught",
        Value = false,
        Callback = function(state)
    ...
    fishFav:Button({
        Title = "Enable Favorite Fish In Inventory",
        Callback = function()
            RunAutoFavoriteLoop()
            print ("selesai ok fav")
        end
    })
    
    fishFav:Button({
        Title = "Enable UnFavorite in Inventory",
        Callback = function()
            RunAutoUnfavoriteLoop()
            print ("selesai ok UnFav")
        end
    })
...
do
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            print(fishRarityUpper,fishName, mutationDisplay)

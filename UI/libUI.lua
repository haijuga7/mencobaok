-- ============================================
-- HYPER-OPTIMIZED CONFIG SYSTEM (Per-Account Folders)
-- ============================================
local DiscordLib = {}
...
DiscordLib.ConfigSystem = {
    ...
    LastConfigFile = "LastConfig.json", -- ✅ Simplified name
...
-- ✅ HELPER: Get player-specific folder path
local function GetPlayerFolder()
    ...
    if not isfolder(playerFolder) then
        ...
        print("[Config] ✅ Created player folder:", playerFolder)
    end
...
-- ✅ ASYNC SAVE: Queue-based non-blocking save (Per-Player)
        ...
        spawn(function()
            ...
            while #self._saveQueue > 0 do
                ...
                local success = pcall(function()
                    ...
                    local filePath = playerFolder .. "/" .. saveData.name .. ".json"
                    writefile(filePath, encoded)
                    
                    print("[Config] ✅ Saved:", saveData.name, "→", filePath)
                end)
...
function DiscordLib.ConfigSystem:LoadConfig(name, silentMode)
    ...
    spawn(function()
        local success, data = pcall(function()
            local playerFolder = GetPlayerFolder()
            local filePath = playerFolder .. "/" .. name .. ".json"
            
            if not isfile(filePath) then
                error("Config not found: " .. filePath)
            end
        ...
        -- ✅ PHASE 2: Load remaining elements
...
function DiscordLib.ConfigSystem:AutoLoadConfig(name)
    local success = pcall(function()
        ...
        print("[Config] ✅ Auto-load set:", name)
    end)
    
    return success
end

-- ✅ GET LAST CONFIG (Per-Player)
...
function DiscordLib.ConfigSystem:ListConfigs()
    ...
    pcall(function()
        ...
        if isfolder(playerFolder) then
            for _, file in ipairs(listfiles(playerFolder)) do
                if file:match("%.json$") and not file:match(self.LastConfigFile) then
                    -- Extract config name from path
                    local configName = file:gsub(playerFolder .. "/", ""):gsub("%.json$", "")

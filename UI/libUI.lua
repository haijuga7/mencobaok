-- ============================================
-- HYPER-OPTIMIZED CONFIG SYSTEM (Handle Massive Configs)
-- ============================================
local DiscordLib = {}
...
-- ✅ ASYNC SAVE: Queue-based non-blocking save
function DiscordLib.ConfigSystem:SaveConfig(name)
    ...
    if not self._saveInProgress then
        ...
        spawn(function()
            task.wait(0.05) -- Micro delay untuk batch
            ...
            while #self._saveQueue > 0 do
                ...
                local success = pcall(function()
                    ...
                    writefile(self.ConfigFolder.."/"..saveData.name..".json", encoded)
...
local ptch = self.ConfigFolder .. "/" .. plr

function DiscordLib.ConfigSystem:AutoLoadConfig(name)
    if not isfolder(ptch) then makefolder(ptch) end
    
    local path_auto = ptch .. "/" self.LastConfigFile
    
    return pcall(function()
        writefile(path_auto, name)
    end)
end

function DiscordLib.ConfigSystem:GetLastConfig()
    local success, configName = pcall(function()
        if isfile(ptch .. "/" self.LastConfigFile) then
            return readfile(ptch .. "/" self.LastConfigFile)
        end
        return nil
    end)

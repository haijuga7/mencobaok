local DiscordLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/haijuga7/mencobaok/refs/heads/main/UI/discordUI.lua"))()
...
do
    ...
    local autoERod = fishSupport:Toggle("autoerod", {
        ...
        Callback = function(b)
            autoERodState = b
            if b then
                autoERodConn = task.defer(function()
...
do 
    ...
    local toggleInstant = fishMain:Toggle("toginst", { -- ✅ DIPERBAIKI
        ...
        Callback = function(state)
            ...
            if state then
                instantLoopThread = task.defer(function()
                ...
                if instantLoopThread then
                    task.cancel(instantLoopThread)
                    instantLoopThread = nil
                end
            end
        end
    })
    ...
    local BlatantV1Toggle = fishMain:Toggle("togblatv1", { -- ✅ DIPERBAIKI
        ...
        Callback = function(state)
            ...
            if state then
                blatantFishv1LoopThread = task.defer(function()
                ...
                if blatantFishv1LoopThread then
                    task.cancel(blatantFishv1LoopThread)
                    blatantFishv1LoopThread = nil end
            end
        end
    })
    ...
    local BlatantV2Toggle = fishMain:Toggle("togblatv2", { -- ✅ DIPERBAIKI
        ...
        Callback = function(state)
            ...
            if state then
                blatantFishv2LoopThread = task.defer(function()
...
do
    ...
    local luckTotemConn = nil
    local mutanTotemConn = nil
    ...
    local buyLuckT = merchantTab:Toggle("buyLuckT", {
        ...
        Callback = function(state)
            if state then
                luckTotemConn = task.defer(function()
                    while state do
                        pcall(function() RF_PurchaseMarketItem:InvokeServer(5) end)
                        task.wait(0.5)
                    end
                    luckTotemConn = nil
                end)
            else
                task.cancel(luckTotemConn)
                luckTotemConn = nil
            end
        end
    })
    ...
    local buyMutanT = merchantTab:Toggle("buyMutanT", {
        ...
        Callback = function(state)
            if state then
                Mutation = task.defer(function()
                    while state do
                        pcall(function() RF_PurchaseMarketItem:InvokeServer(8) end)
                        task.wait(0.5)
                    end
                    mutanTotemConn = nil
                end)
            else
                task.cancel(mutanTotemConn)
                mutanTotemConn = nil
            end
        end
    })
...
Fish Secret --> %d / %d]],
    ...
    local function hookFishCaught()
        ...
        for _, Connection in pairs(getconnections(FishCaught.OnClientEvent)) do
            local old; old = hookfunction(Connection.Function, function(...)
                -- ✅ Call original function FIRST
                local result = old(...)
                
                -- ✅ Only count if active
                if fishCountActive then
                    Pola.fish_Count = Pola.fish_Count + 1
                    if Pola.fish_Count >= Pola.fish_Max then
                        oopa_res()
                    end
                end
    ...
    local function hookStandAlone()
        ...
        for _, Connection in pairs(getconnections(StandAlone.OnClientEvent)) do
            local old; old = hookfunction(Connection.Function, function(...)
                -- ✅ Call original function FIRST
                local result = old(...)
                ...
                if fishCountActive then
                    ...
                    -- Only count if it's our character
    ...
    local togPola = polaTab:Toggle("togPola", {
        ...
        Callback = function(state)
            ...
            if state then
                ...
                fishUpdateStat = task.defer(function()
                ...
                print("[Pola] ❌ System deactivated!")
                updateFishStatus()
                task.cancel(fishUpdateStat)
            end
        end
    })

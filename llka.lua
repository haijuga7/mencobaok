...
local StandAlone = NET_PATH['RE/ReplicateCutscene']
...
local function updateHookFish(tier, fish)
    ...
    if tier == 'Legendary' then
        ...
        fishLegendText = fishLegendText .. string.format("%s - Caught on %s\n", fish, timestamp)
        ...
        print(string.format("🌟 LEGENDARY: %s (Total: %d)", fish, fishLegend))
        
    elseif tier == 'Mythic' then
        fishMitos = fishMitos + 1
        fishMitosText = fishMitosText .. string.format("%s - Caught on %s\n", fish, timestamp)
        ...
        print(string.format("✨ MYTHIC: %s (Total: %d)", fish, fishMitos))
        
    else
        fishSecret = fishSecret + 1
        fishSecretText = fishSecretText .. string.format("%s - Caught on %s\n", fish, timestamp)
        ...
        print(string.format("🔮 SECRET: %s (Total: %d)", fish, fishSecret))
    end
end
...
local function startHookFish()
    ...
    HookFishConnection = StandAlone.OnClientEvent:Connect(function(...)
        ...
        if fishLegend >= 3 then
            WindUI:Notify({
                Title = "🌟 3+ Legendary Fish!",
                Content = "Auto-selling and returning...",
                Duration = 1
            })
            ...
            -- Update displays
            if fishResultLegends then
                pcall(function() fishResultLegends:SetDesc("Reset - waiting for new catches") end)
            end
...
local function startFishCounter()
    ...
    fishCounterConnection = FISH_CAUGHT.OnClientEvent:Connect(function(...)
        ...
        -- Log ke console
        print(string.format("🎣 Fish Count: %d/%d | Loop: %d", fishCaughtCount, fishMax, fishLoop))
...
fishFomoSett:Button({
    ...
    Desc = "Reset legendary, mythic, and secret fish data",
    Callback = function()
        fishLegend = 0
        fishMitos = 0
        fishSecret = 0
        ...
        -- Update displays
        if fishResultLegends then
            pcall(function() fishResultLegends:SetDesc("No legendary fish caught yet") end)
        end
        updateFishStatus()
    end
})

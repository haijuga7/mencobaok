...
local function saveOriginalSettings()
    ...
    originalSettings.lighting.effects = {}
    for _, effect in pairs(Lighting:GetChildren()) do
        if effect:IsA("PostEffect") then
            originalSettings.lighting.effects[effect] = effect.Enabled
        end
    end
...
local function aggressiveMemoryOptimization()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        -- Remove/Optimize Meshes (HUGE memory saver)
        ...
        -- Disable Sounds (memory hog)
        if obj:IsA("Sound") then
            obj.Volume = 0
            obj:Stop()
            optimized.sounds = optimized.sounds + 1
        end
    ...
    -- Optimize PlayerGui (UI textures)
    if player.PlayerGui then
        for _, gui in pairs(player.PlayerGui:GetDescendants()) do
            if gui:IsA("ImageLabel") or gui:IsA("ImageButton") then
                -- Keep UI functional but reduce quality
                gui.ImageTransparency = 0.3
                optimized.gui = optimized.gui + 1
            end
        end
    end
    
    print(string.format("✓ Optimized: %d meshes, %d textures, %d sounds, %d UI elements", 
        optimized.meshes, optimized.textures, optimized.sounds, optimized.gui))
end
...
local function enableOptimization()
    ...
    -- 3. Simplify lighting (reduce calculations)
    ...
    -- 6. Optimize all parts (MAJOR memory saver)
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            obj.CastShadow = false
            obj.Material = Enum.Material.SmoothPlastic
            
            -- Remove reflectance (memory saver)
            obj.Reflectance = 0
    ...
    -- 9. Reduce render distance (HUGE memory saver)
    if sethiddenproperty then
        pcall(function()
            sethiddenproperty(workspace, "StreamingMinRadius", 5)
    ...
    pcall(function()
        ...
        local after = gcinfo()
        print(string.format("✓ Garbage collected: %.2f MB freed", (before - after) / 1024))
    end)
    
    print("✅ AGGRESSIVE OPTIMIZATION ENABLED!")
    print("⚡ Graphics: ULTRA LOW | Memory: MINIMIZED")
end

-- ========================================
-- DISABLE OPTIMIZATION (RESTORE)
-- ========================================

local function disableOptimization()
    print("\n🔧 DISABLING OPTIMIZATION (RESTORING)...")
    ...
    for effect, enabled in pairs(originalSettings.lighting.effects) do
    ...
    if setfpscap then
        setfpscap(60)
        print("✓ FPS restored to 60")
    end
    
    print("✅ OPTIMIZATION DISABLED!")
    print("🎨 Graphics: NORMAL | Original settings restored")
end
...
local function toggleFPSBoost()
    if not optimizationEnabled then
...
local function startFishCounter()
    ...
    fishCounterConnection = FISH_CAUGHT.OnClientEvent:Connect(function(...)
        ...
        if fishCaughtCount == fishMax then
            ...
            fishLegend = 0
            fishMitos = 0
            fishMaxCought = fishMaxCought + 1
            
            if fishMaxCought >= 5 then
                sendWebHookTest()
            end

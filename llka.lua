...
local function isLocalPlayer(targetPlayer)
    if targetPlayer == player then
        return true
    end
    
    if typeof(targetPlayer) == "Instance" and targetPlayer:IsA("Player") then
        return targetPlayer.UserId == player.UserId
    end
...
local function startFishCounter()
    ...
    fishCounterConnection = CAUGHT_FISH_VISUAL.OnClientEvent:Connect(function(...)
        local args = {...}
        ...
        local fishDataTable = args[4]
        
        -- Filter: Skip jika bukan local player
        if not isLocalPlayer(targetPlayer) then
            return
        end
...
-- Paragraph untuk status (tanpa function di Desc - INI PENTING!)
...
fishFomoSett:Toggle({
    ...
    Description = "Auto sell when max fish reached",
    Value = false,
    Callback = function(state)
        if state then
            startFishCounter()
